...
/// Simple full-coverage liquid glass widget
class SimpleFullCoverageWidget extends StatefulWidget {
  final Widget child;
  final double? width;
  final double height;
  ...
  const SimpleFullCoverageWidget({
    super.key,
    required this.child,
  ...
  @override
  State<SimpleFullCoverageWidget> createState() => _SimpleFullCoverageWidgetState();
}

class _SimpleFullCoverageWidgetState extends State<SimpleFullCoverageWidget> 
  ...
  Future<void> _initializeWebShader() async {
    ...
    try {
      debugPrint('🔍 [SimpleFullCoverage] Loading full coverage shader...');
      final program = await ui.FragmentProgram.fromAsset('shaders/full_coverage_liquid_glass.frag');
      _shader = program.fragmentShader();
      debugPrint('🔍 [SimpleFullCoverage] Shader loaded successfully');
      ...
      debugPrint('❌ [SimpleFullCoverage] Error loading shader: $e');
    }
  }

  Future<void> _captureBackground() async {
    if (!_isInitialized || widget.backgroundKey.currentContext == null) {
      debugPrint('🔍 [SimpleFullCoverage] Skipping background capture - not ready');
      return;
    }

    try {
      debugPrint('🔍 [SimpleFullCoverage] Capturing background...');
      ...
      debugPrint('🔍 [SimpleFullCoverage] Background captured successfully');
      ...
      debugPrint('❌ [SimpleFullCoverage] Error capturing background: $e');
    }
  }
  ...
  Widget _buildLiquidGlassEffect() {
    ...
    return CustomPaint(
      ...
      painter: _SimpleFullCoveragePainter(
        shader: _shader!,
...
/// Custom painter for simple full-coverage liquid glass effect
class _SimpleFullCoveragePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    debugPrint('🔍 [SimpleFullCoverage] Painting liquid glass effect');
...
/// Predefined simple full-coverage styles
class SimpleFullCoverageStyles {
  static Widget small({
    ...
    return SimpleFullCoverageWidget(
      width: 80,
      height: 80,
      borderRadius: 12.0,
